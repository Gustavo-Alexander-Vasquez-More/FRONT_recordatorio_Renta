...
<div className='flex flex-col bg-[#ececec] w-full lg:h-[90vh]'>
    ...
    <div className="flex flex-col bg-[white] py-[1.5rem] px-[1.5rem] gap-1 w-full overflow-x-hidden overflow-y-auto min-h-[52vh] max-h-[52vh] ">
  ...
  <div
    className="w-full border-b-[1px] overflow-x-auto overflow-y-hidden border-solid flex justify-between items-center px-[1rem] py-[0.5rem]"
    key={dat._id}
  >
    <div className="flex w-full gap-2 lg:gap-4 items-center min-w-[180px] max-w-[300px]">
      {dat.foto && (
        <img
          className="w-[3rem] h-[3rem] border-solid border-1 rounded-full"
          src={dat.foto}
          alt=""
        />
      )}
      {!dat.foto && (
        <div
          className="w-[3rem] h-[3rem] rounded-full bg-gray-100 border-solid border-1 text-white flex justify-center items-center"
      ...
      <p className="text-sm lg:text-base">{dat.usuario}</p>
    </div>
    <div className="flex gap-3 min-w-[250px]">
      <button
        className="bg-primary text-white rounded-[5px] px-[1rem] py-[0.3rem] flex gap-1"
        onClick={() => {
          openModal(), setUserSelected(dat._id);
        }}
      >
        <svg
      ...
      <button
        className="bg-[red] text-white rounded-[5px] px-[1rem] py-[0.3rem] flex gap-1"
        onClick={() => deleteUser(dat.usuario)}
      >
        <svg
          className="w-6 h-6 text-white"
          aria-hidden="true"
